...
describe('layoutGraph – two disconnected subgraphs', () => {
  ...
  it('respects direction for stacking (TD = horizontal)', async () => {
    ...
    // In TD mode, subgraphs should be stacked horizontally (perpendicular to flow)
    // One should be to the left of the other
    const isHorizontallyArranged =
      (s1.x + s1.width <= s2.x) || (s2.x + s2.width <= s1.x)

    expect(isHorizontallyArranged).toBe(true)
  })
})
